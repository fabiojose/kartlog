...
public class LogService {
	...
	/*
	public void report(String filePathName, final OutputStream output) throws FileNotFoundException, IOException {
		requireNonNull(filePathName);
		requireNonNull(output);
			...
			.forEach(ranking -> {
				try { 
					output.write(String.format(
						"|%20d|%-20s|%-20s|%20d|%20s|\n",
						ranking.getPosicao(),
						ranking.getPiloto().getCodigo(),
						ranking.getPiloto().getNome(),
						ranking.getVoltasCompletadas(),
						
						String.format("%d:%02d:%02d.%03d",
							ranking.getTempoTotalProva().toHoursPart(),
							ranking.getTempoTotalProva().toMinutesPart(),
							ranking.getTempoTotalProva().toSecondsPart(),
							ranking.getTempoTotalProva().toMillisPart()
						)
					).getBytes());
				}catch(IOException e) {
					log.error(e.getMessage(), e);
